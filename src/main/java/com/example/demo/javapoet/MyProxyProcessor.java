...
class MyProxyProcessor implements BeanPostProcessor, BeanRegistrationAotProcessor {
		...
		return (context, code) -> {
			...
			var rayInitGeneratedMethod = code.getMethods().add("rayInit",
					builder -> builder.addModifiers(Modifier.STATIC).addParameter(RegisteredBean.class, "rb")
							...
							.addCode(CodeBlock.builder().addStatement("System.out.println(\"Hello, world\")")
									.addStatement("return gs ").build()));
			code.addInstancePostProcessor(
					MethodReference.ofStatic(code.getClassName(), rayInitGeneratedMethod.getName()));
		};
	}
	...
	private static Object proxyForRayAnnotation(Method[] methodsWithAnnotation, Object target) {
		...
		pfb.addAdvice((MethodInterceptor) invocation -> {
			...
			var methodPredicate = (Predicate<Method>) theMethod -> theMethod.getName().equals(method.getName())
					&& Arrays.equals(theMethod.getParameterTypes(), method.getParameterTypes())
					&& theMethod.getReturnType().equals(method.getReturnType());
