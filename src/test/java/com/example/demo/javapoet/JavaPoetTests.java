...
class JavaPoetTests {
	...
	private MethodSpec buildSubclassMethodFor(Method method) {
		var newMethodDefinition = MethodSpec.methodBuilder(method.getName()).addModifiers(Modifier.PUBLIC)
	...
	private TypeSpec subclassFor(Class<?> target) {
		...
		var methods = Stream.of(ReflectionUtils.getUniqueDeclaredMethods(target)).filter(
				m -> !java.lang.reflect.Modifier.isPrivate(m.getModifiers()) && !ReflectionUtils.isObjectMethod(m))
				.map(this::buildSubclassMethodFor).toList();
		var newType = TypeSpec.classBuilder(target.getName() + "__Proxy__" + random()).addModifiers(Modifier.PUBLIC);
